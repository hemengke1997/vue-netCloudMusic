<template>
  <section class="mvdes" @click="showdes = !showdes" :class="{videodes: !ismv}">
    <h4 class="mv_title">
      <p class="name">
        <i class="ignore_icon_mv" v-if="ismv">MV</i>
        <span>{{des.name}}</span>
        <i class="iconfont icon-sanjiaoxing" :class="{showdes:showdes}" v-if="desc_length"></i>
      </p>
    </h4>
